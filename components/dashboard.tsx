...
export function Dashboard({ user, onLogout }: DashboardProps) {
  ...
  const fetchLeaderboard = useCallback(async () => {
    ...
    if (!error && data) {
      ...
      const res = mapped?.filter((item) => item.is_player) ?? [];
      setPlayers(res);
        ...
        .map((t) =>
          t.id === commentId
            ? {
                ...
                likeCount: t.likedByMe ? t.likeCount - 1 : t.likeCount + 1,
              }
            : t,
        )
        ...
        p.id === updated.id
          ? {
              ...
              position: updated.position
                ? (POSITION_CN[updated.position] ?? p.position)
                : p.position,
    ...
    setSelectedPlayer((prev) =>
      prev?.id === updated.id
        ? {
            ...
            name: updated.full_name,
            position: updated.position
              ? (POSITION_CN[updated.position] ?? prev.position)
              : prev.position,
            avatar_url: updated.avatar_url,
          }
        : prev,
    ...
    <div className="min-h-screen pb-8">
      ...
      {selectedPlayer && (
        <PlayerModal
          ...
          trashTalks={trashTalks.filter(
            (t) => t.playerId === selectedPlayer.id,
          )}
