...
const PARTICLE_POSITIONS = [
  { x: 10, y: 15 }, { x: 85, y: 25 }, { x: 45, y: 80 }, { x: 20, y: 60 },
  { x: 70, y: 10 }, { x: 55, y: 45 }, { x: 30, y: 90 }, { x: 90, y: 70 },
  { x: 15, y: 35 }, { x: 60, y: 55 }, { x: 40, y: 20 }, { x: 75, y: 85 },
  { x: 25, y: 50 }, { x: 80, y: 40 }, { x: 50, y: 75 }, { x: 5, y: 95 },
  { x: 65, y: 30 }, { x: 35, y: 65 }, { x: 95, y: 5 }, { x: 12, y: 88 },
]
...
export function LoginCard() {
  ...
  const handleSubmit = async (e: React.SyntheticEvent<HTMLFormElement>) => {
    ...
    if (!email) { setError(">> ERROR: 请输入邮箱地址"); return }
    if (!validateEmail(email)) {
      setError(">> ACCESS DENIED: 非研究所协议检测到。仅允许 *.ac.cn / ucas.ac.cn / mails.ucas.edu.cn / cstnet.cn 域名")
      return
    }
        ...
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          {PARTICLE_POSITIONS.map((pos, i) => (
            <motion.div
              ...
              transition={{ duration: 3 + (i % 5) * 0.4, repeat: Infinity, repeatType: "reverse", delay: (i % 10) * 0.2 }}
            />
          ))}
        </div>
      )}
      ...
      >
        ...
        <div className="glass-panel rounded-xl p-8 relative">
          ...
          <div className="text-center mb-6">
            ...
            <h1 className="text-2xl font-bold text-foreground neon-text">[CAS] BALLER SYSTEM</h1>
          ...
          {sent ? (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center space-y-4">
              <div className="p-4 rounded-lg bg-primary/10 border border-primary/30">
                <p className="text-primary font-mono text-sm">{">> MAGIC LINK DISPATCHED"}</p>
                <p className="text-muted-foreground text-xs mt-2 font-mono">请检查邮箱 {email}，点击链接完成登录</p>
              </div>
            </motion.div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-2">
                <label className="text-xs font-mono text-muted-foreground uppercase tracking-wider">{">"} 邮箱地址</label>
              ...
              {mode === "password" && (
                <div className="space-y-2">
                  <label className="text-xs font-mono text-muted-foreground uppercase tracking-wider">{">"} 密码</label>
                  <Input
                    type="password"
                    placeholder="••••••••"
                    value={password}
              ...
              {error && (
                <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} className="p-3 rounded-lg bg-destructive/10 border border-destructive/30">
              ...
              >
                {isLoading ? (
                  <><Loader2 className="w-4 h-4 mr-2 animate-spin" />{mode === "otp" ? "正在发送魔法链接..." : "验证中..."}</>
                ) : (
                  <>
